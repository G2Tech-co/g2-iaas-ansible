...
backup_dir="/mnt/storage/backups/mysql"
db_backup="/mnt/storage/backups/mysql/dumps-$(date -I)"
db_user="root"
hostname=$(hostname)
...
for db in $(mysql --user=$db_user -e 'show databases' -s --skip-column-names | grep -vi information_schema); do
    mysqldump --user="$db_user" --single-transaction --opt "$db" | gzip > "$db_backup"/mysql-"$hostname"-"$db"-$(date +%Y-%m-%d).gz;
done

mysqldump --user="$db_user" --opt --single-transaction performance_schema | gzip > "$db_backup"/mysql-"$hostname"-performance_schema-1-$(date +%Y-%m-%d).gz
